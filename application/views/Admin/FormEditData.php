...
<head profile="http://gmpg.org/xfn/11">
	...
	$(function(){
		$('#suggest').autocomplete('<?=base_url();?>index.php/kru/SearchMentor/', 
			{
			parse: function(data){ 
					var parsed = [];
					for (var i=0; i < data.length; i++) {
						parsed[i] = {
							data: data[i],
							value: data[i].Nama_Lengkap // nama field yang dicari
						};
					}
					return parsed;
				},
				formatItem: function(data,i,max){
					...
					str += '<u>'+data.Nama_Lengkap+'</u><br />'+data.Jurusan+'<br />NIM: '+data.NIM;
					str += '</div>';
					return str;
				},
...
<form action="<?=base_url();?>index.php/kru/EditMentor/<?=$row->NIM;?>" method="POST">

  <fieldset>
    <legend>Edit Data Cakru</legend>
	
	<label> Id Database : <?=$row->id;?> </label>
	
    <label>Nama Lengkap : <?=$row->Nama_Lengkap;?></label>
    
    <label>Jurusan : <?=$row->Jurusan;?></label>
	
	<label> NIM : <?=$row->NIM;?> </label>
    
	<input type="hidden" name="IsSubmit" value="TRUE">
	
     <label>Mentor</label>
    <input type="text" name="mentor" value="<?=$row->mentor;?>" id="suggest">
    ...
    <span class="help-block">1 artinya sudah bayar, 0 artinya belum</span>
