...
<head profile="http://gmpg.org/xfn/11">
	...
	$(function(){
		$('#suggest').autocomplete('<?=base_url();?>index.php/kru/SearchCakru/', 
			{
			parse: function(data){ 
					var parsed = [];
				...
				formatItem: function(data,i,max){
					...
					str += '<u>Nama : '+data.Nama_Lengkap+'</u><br />NIM : '+data.NIM;
					str += '</div>';
					return str;
				},
			...
			{
				$("#suggest").val(data.NIM);
			}// fungsi ini akan dipanggil setelah user memilih salah satu result pencarian autocomplete
		);
	});
  </script>
...
<form action = "<?=base_url();?>index.php/kru/aktivasi" method="POST">
  <fieldset>
	...
	<label><?=$pesan;?></label>
	<label>NIM</label>
    <input type="text" name="NIM" id="suggest">
