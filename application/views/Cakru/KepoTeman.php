...
<head profile="http://gmpg.org/xfn/11">
	...
	$(function(){
		$('#suggest').autocomplete('<?=base_url();?>index.php/cakru/SearchTeman/', 
			{
			parse: function(data){ 
					var parsed = [];
					for (var i=0; i < data.length; i++) {
						parsed[i] = {
							data: data[i],
							value: data[i].NIM // nama field yang dicari
						};
					}
					return parsed;
				},
				formatItem: function(data,i,max){
					...
					str += '<u>'+data.Nama_Lengkap+'</u><br />'+data.NIM+'<br />Telp: '+data.Hape;
					str += '</div>';
					return str;
				},
			...
			function(event,data,formated){
				$('#CakruDetail').show(); // tampilan table detail yang di hide sebelumnya
				$('#dataNIM').text(data.NIM); 
				$('#dataNama').text(data.Nama_Lengkap);
				$('#dataJurusan').text(data.Jurusan);
				$('#dataHape').text(data.Hape);
				$('#dataDivisi').text(data.divisi);
				...
				{
					$('#dataStatus').text("Cakru");
				}
				
				// tampilkan foto pada element <img id="dataFoto">, apabila foto kosong, maka tampilkan default unknown image 
				$('#dataFoto').attr('src',data.Foto);
			}
		);
	});
  </script>	
	
	<link rel="stylesheet" href="<?=base_url();?>css/jquery.autocomplete.css" type="text/css">
	<link rel="stylesheet" href="<?=base_url();?>css/custom_search.css" type="text/css">
...
<p style="color:blue">Cari <b>NIM</b> Cakru/Kru: </p><input type="text" id="suggest"/><br /><br />
<table id="CakruDetail" style="display:none">
	<tr>
		<td colspan="7"><div class="detail_title">Detail Cakru/Kru</div></td>
	</tr>
	
	<tr valign="top">
		<td rowspan="6"><img src="" id="dataFoto" height="200px" width="200px"/></td>
		<td><b>NIM</b></td>
		<td>:</td>
		<td id="dataNIM"></td>
	</tr>
