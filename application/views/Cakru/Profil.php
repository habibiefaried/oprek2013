...
<div id="profil">
  <fieldset>
	...
	<label><img src="<?=$this->session->userdata('Foto');?>" height="200px" width="200px"></label>
	<label>Nama Lengkap : <?=$this->session->userdata('Nama_Lengkap');?></label>
	<label>Email : <?=$this->session->userdata('Email');?></label>
	<label>NIM : <?=$this->session->userdata('NIM');?></label>
	<label>No Handphone : <?=$this->session->userdata('Hape');?> </label>
	<label>Jurusan : <?=$this->session->userdata('Jurusan');?></label>
	<label>Alamat : <?=$this->session->userdata('Alamat');?></label>
	<label>Mentor : <?=$this->session->userdata('mentor');?></label>
	
  </fieldset>
</div>

<div id="editprofil">
	<?php echo form_open_multipart("cakru/profil");?>
	<fieldset>
	<legend> Edit Profil </legend>
	
	<input type="hidden" name="id" value="<?=$this->session->userdata('id');?>">
	
	<label><b> NIM : <?=$this->session->userdata('NIM');?> </b></label>
	
	<label> Nama Lengkap : </label>
	<input type="text" name="NamaLengkap" value="<?=$this->session->userdata('Nama_Lengkap');?>">
	
	<label> Email : </label>
	<input type="text" name="Email" value="<?=$this->session->userdata('Email');?>">

	<label> Jurusan : </label>
	<input type="text" name="Jurusan" value="<?=$this->session->userdata('Jurusan');?>">
	
	<label> No Handphone : </label>
	<input type="text" name="Hape" value="<?=$this->session->userdata('Hape');?>">
	...
	<span class="help-block">Masukkan password jika ingin diganti, kosongkan jika tidak ingin diganti</span>
	
	<br>
	<img src="<?=$this->session->userdata('Foto');?>" height="300px" width="300px"><br>
	<label> Foto: </label>
